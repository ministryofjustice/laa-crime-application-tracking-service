...
class MockWebServerStubsTest {
    ...
    void verifyThatThereAreNoDuplicatePathMappings() {
        MockWebServerStubs.RequestPathResponseMapping[] requestPathResponseMappings =
                MockWebServerStubs.RequestPathResponseMapping.values();
        Map<String, Integer> requestPathOccurrences = new HashMap<>();

        for (MockWebServerStubs.RequestPathResponseMapping requestPathResponseMapping :
                requestPathResponseMappings) {
        ...
        List<Map.Entry<String, Integer>> nonUniqueRequestPaths =
                requestPathOccurrences.entrySet().stream()
                        .filter(stringIntegerEntry -> stringIntegerEntry.getValue() > 1)
                        .toList();

        if (!nonUniqueRequestPaths.isEmpty()) {
            fail(
                    "Expected all requestPaths to be unique but found the following duplicates: "
                            + nonUniqueRequestPaths);
        }
    }
}